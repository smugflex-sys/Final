...
// Get teacher's students for a specific class - UPDATED VERSION
  ...
  const getParentChildren = (parentId: number): any[] => {
    ...
    const linkParentId = typeof parentId === 'string' ? parseInt(parentId) : parentId;
    const parentStudentLinks = Array.isArray(parentStudentLinksData) ? parentStudentLinksData.filter((link: any) => 
    ...
    try {
      ...
      if (currentUser?.role === 'parent') {
        ...
        if (linksArray && linksArray.length > 0) {
          ...
          // No parent-student links found for parent ID
        }
      } else {
